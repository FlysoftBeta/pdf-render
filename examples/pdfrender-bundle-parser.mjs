...
function parseBundle(buffer) {
  ...
  const u8 = () => take(1)[0];
  const u16 = () => { const value = buffer.readUInt16LE(offset); take(2); return value; };
  const u32 = () => { const value = buffer.readUInt32LE(offset); take(4); return value; };
  const u64 = () => {
    const value = buffer.readBigUInt64LE(offset);
    take(8);
    if (value > BigInt(Number.MAX_SAFE_INTEGER)) fail('record is too large for Node.js');
    return Number(value);
  };
  ...
  for (let i = 0; i < blobCount; ++i) {
    ...
    if (createHash('sha256').update(data).digest('hex') !== id) fail(`hash mismatch for blob ${id}`);
    if (encoding === 1 && !mime.startsWith('text/')) fail(`non-text blob ${id} is compressed`);
    if (encoding === 0 && mime.startsWith('text/')) fail(`text blob ${id} is not compressed`);
  ...
  for (let i = 0; i < fileCount; ++i) {
    ...
    if (!name || name.includes('\\') || path.posix.isAbsolute(name) || name.split('/').includes('..')) {
...
function rewriteReferences(file, filesById) {
  if (!file.mime.startsWith('text/') && file.mime !== 'image/svg+xml') return file.data;
  ...
  for (const [id, target] of filesById) {
    ...
    text = text.replace(new RegExp(`(src|href|data)="${escapedId}"`, 'g'), `$1="${relative}"`);
  }
...
async function main() {
  ...
  console.log(`Extracted ${parsed.files.length} files (${parsed.blobs.size} unique blobs, ${parsed.dictionarySize} dictionary bytes) to ${outputName}`);
}
